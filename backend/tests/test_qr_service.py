...
class DummyDetector:
    ...
    def __init__(
        self,
        *,
        single_result: Optional[Tuple[str, object, object]] = None,
        multi_result: Optional[Tuple[bool, list, object, object]] = None,
    ) -> None:
...
def test_detect_qr_single_returns_text(monkeypatch) -> None:
...
def test_detect_qr_single_returns_none_when_no_text(monkeypatch) -> None:
...
def test_detect_qr_multi_returns_first_valid_text(monkeypatch) -> None:
...
def test_detect_qr_multi_returns_none_when_no_codes(monkeypatch) -> None:
...
def test_detect_qr_with_points_returns_text_and_points(monkeypatch) -> None:
...
def test_decode_qr_from_bytes_returns_decoded_text(monkeypatch) -> None:
    """decode_qr_from_bytes should decode the image and return the decoded QR text."""
    module = importlib.import_module("backend.services.qr_service")
    # patch np.frombuffer to return a numpy array
    monkeypatch.setattr(
        module.np, "frombuffer", lambda b, dtype: np.array([1, 2, 3], dtype=np.uint8)
    )
    # patch cv2.imdecode to return a dummy image array
    monkeypatch.setattr(
        module.cv, "imdecode", lambda arr, flags: np.zeros((10, 10, 3), dtype=np.uint8)
    )
...
def test_decode_qr_from_bytes_handles_decode_failure(monkeypatch) -> None:
