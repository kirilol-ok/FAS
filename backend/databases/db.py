...
postgres_engine = create_async_engine(
    POSTGRES_URL, echo=True, future=True, connect_args={"ssl": False}
)
...
async def init_postgres_db() -> None:
    ...
    print(
        " Nie udało się połączyć z bazą po wielu próbach. Aplikacja może nie działać poprawnie."
    )
    ...
    async with async_session() as db:
        try:
            ...
            result = await db.execute(
                select(Admins).where(Admins.email == "admin@example.com")
            )
