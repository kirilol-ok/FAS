...
REPORT_STATUS_ENUM = postgresql.ENUM(
    "OK", "Error", name="report_status", create_type=False
)
...
def upgrade() -> None:
    ...
    postgresql.ENUM("OK", "Error", name="report_status").create(
    ...
    op.create_table(
        ...
        sa.Column(
            "image_id", sa.Integer(), sa.ForeignKey("ImageFiles.id"), nullable=True
        ),
    )
    op.create_index(
    ...
    op.create_index(
        op.f("ix_Reports_employee_id"), "Reports", ["employee_id"], unique=False
    )
...
def downgrade() -> None:
    ...
    postgresql.ENUM("OK", "Error", name="report_status").drop(
        op.get_bind(), checkfirst=True
    )
