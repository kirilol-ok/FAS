    ...
    if ($('#loginForm').length) {
        $('#loginForm').on('submit', function(e) {
            ...
            loginUser(credentials)
                ...
                .fail(function(xhr) {
                    if (xhr.status === 401) {
                        $('#errorMessage').text('Invalid login or password').addClass('show');
                    } else {
                        $('#errorMessage').text('Connection error. Please try again.').addClass('show');
                    }
                });
        });
    }
        ...
        getWorkersTable()
            .done(function(workers) {
                ...
                $('.btn-edit-worker').on('click', function() {
                    ...
                    alert(`Editing employee ID: ${employeeId}\n\nIn full version redirect to edit.html?id=${employeeId}`);
                });
            })
            .fail(function(xhr) {
                $('#workersTableContainer').html('<p class="loading">Error loading employee data.</p>');
                console.error('Error fetching employees:', xhr);
            });

        $('#editBtn').on('click', function() {
            alert('Redirecting to employee edit page\n\nIn full version: window.location.href = "edit.html"');
        });
